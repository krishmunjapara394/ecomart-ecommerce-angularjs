  ...
  ngOnInit() {
    this.userService.getCart().subscribe({
      next: (products) => {
        ...
        // Fix: multiply price by count for correct total
        this.order.totalPrice = this.cart.reduce(
          (acc: number, product: any) => acc + (product.price * product.count),
          0
        );
      },
      error: (error) => {
    ...
    if (this.currentStep <= 4) {
      ...
      if (
        ...
        Swal.fire({
          ...
          text: 'Please fill in all payment fields before continuing.',
          confirmButtonColor: '#4c1d95',
        });
        return;
      }
